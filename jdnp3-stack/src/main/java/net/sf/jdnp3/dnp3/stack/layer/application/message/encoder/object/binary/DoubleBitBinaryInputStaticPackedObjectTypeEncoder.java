...
public class DoubleBitBinaryInputStaticPackedObjectTypeEncoder implements ObjectTypeEncoder {
	public boolean canEncode(FunctionCode functionCode, ObjectType objectType) {
		return FunctionCodeUtils.isResponseCode(functionCode) && objectType.equals(DOUBLE_BIT_BINARY_INPUT_STATIC_PACKED);
	}
	...
	public void encode(ObjectFragmentEncoderContext context, ObjectInstance objectInstance, List<Byte> data) {
		...
		long bitIndex = 2*(context.getCurrentIndex() - context.getStartIndex()) % 8;
		if (bitIndex == 0) {
			data.add((byte) 0);
		}
		
		byte value = data.get(data.size() - 1);
		DoubleBitBinaryInputStaticObjectInstance specificObjectInstance = (DoubleBitBinaryInputStaticObjectInstance) objectInstance;
		value |= (specificObjectInstance.getValue() << bitIndex);
